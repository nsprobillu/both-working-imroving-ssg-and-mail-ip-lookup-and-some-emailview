...
export function Monitor() {
  ...
  // Email IP lookup state
  const [emailIpLookup, setEmailIpLookup] = useState('');
  const [emailIpLookupResult, setEmailIpLookupResult] = useState<TempEmailLookupResult | null>(null);
  const [isEmailIpLookupLoading, setIsEmailIpLookupLoading] = useState(false);
  const [emailIpLookupError, setEmailIpLookupError] = useState('');
  const [emailIpLookupSuccess, setEmailIpLookupSuccess] = useState(false);
  ...
  // Email IP lookup function
  const handleEmailIpLookup = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!emailIpLookup) {
      setEmailIpLookupError('Please enter a temporary email address');
      return;
    }
    
    setIsEmailIpLookupLoading(true);
    setEmailIpLookupError('');
    setEmailIpLookupSuccess(false);
    setEmailIpLookupResult(null);
    
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/monitor/lookup-email-ip`,
        { 
          params: { email: emailIpLookup },
          headers: { 'Admin-Access': import.meta.env.VITE_ADMIN_PASSPHRASE }
        }
      );
      
      if (response.data) {
        setEmailIpLookupResult(response.data);
        setEmailIpLookupSuccess(true);
      } else {
        setEmailIpLookupError('No IP information found for this temporary email');
      }
    } catch (error: any) {
      console.error('Email IP lookup failed:', error);
      setEmailIpLookupError(error.response?.data?.error || 'Failed to lookup email IP');
    } finally {
      setIsEmailIpLookupLoading(false);
    }
  };
  ...
  return (
   ...
   <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          {/* Email IP Lookup Form */}
          <div className="bg-gray-800 rounded-lg p-6">
            <h2 className="text-xl font-bold mb-4">Lookup Email IP</h2>
            <form onSubmit={handleEmailIpLookup} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">
                  Temporary Email
                </label>
                <div className="flex">
                  <input
                    type="email"
                    value={emailIpLookup}
                    onChange={(e) => setEmailIpLookup(e.target.value)}
                    className="flex-1 bg-gray-700 text-white border border-gray-600 rounded-l px-4 py-2 focus:outline-none focus:border-blue-500"
                  ...
                  <button
                    type="submit"
                    className="bg-blue-600 text-white rounded-r px-6 py-2 hover:bg-blue-700 transition-colors flex items-center"
                    disabled={isEmailIpLookupLoading}
                  >
                    {isEmailIpLookupLoading ? (
                      <RefreshCw className="w-5 h-5 animate-spin" />
                    ) : (
                      <Search className="w-5 h-5" />
                    )}
            ...
            {emailIpLookupError && (
              <div className="mt-4 bg-red-500/10 border border-red-500 text-red-500 p-3 rounded">
                {emailIpLookupError}
              </div>
            )}

            {emailIpLookupSuccess && emailIpLookupResult && (
              <div className="mt-4 bg-green-500/10 border border-green-500 p-4 rounded">
                <div className="flex items-center mb-2">
                  <CheckCircle className="w-5 h-5 text-green-500 mr-2" />
                  <h3 className="font-bold">IP Lookup Result:</h3>
                </div>
                <div className="grid grid-cols-1 gap-4 mt-3">
                  <div>
                    <p className="text-sm text-gray-400">Temporary Email:</p>
                    <p className="font-mono">{emailIpLookupResult.email}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-400">Owner's Email:</p>
                    <p className="font-mono text-green-400 font-bold">{emailIpLookupResult.owner_email}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-400">IP Address:</p>
                    <p className="font-mono text-blue-400">{emailIpLookupResult.client_ip}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-400">First Seen:</p>
                    <p>{new Date(emailIpLookupResult.first_seen).toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-400">Last Seen:</p>
                    <p>{new Date(emailIpLookupResult.last_seen).toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-400">Expires At:</p>
                    <p>{new Date(emailIpLookupResult.email_expires_at).toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-400">Request Count:</p>
                    <p>{emailIpLookupResult.request_count}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-400">Behavior Score:</p>
                    <p>{emailIpLookupResult.behavior_score}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-400">Status:</p>
                    <p className={emailIpLookupResult.is_suspicious ? "text-red-500" : "text-green-500"}>
                      {emailIpLookupResult.is_suspicious ? "Suspicious" : "Normal"}
                    </p>
                  </div>
                </div>
              </div>
